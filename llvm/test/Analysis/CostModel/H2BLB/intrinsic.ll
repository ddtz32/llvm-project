; RUN: opt -passes="print<cost-model>" -cost-kind=code-size -disable-output %s \
; RUN:   2>&1 | FileCheck %s --check-prefix=SIZE

; RUN: opt -passes="print<cost-model>" -cost-kind=latency -disable-output %s \
; RUN:   2>&1 | FileCheck %s --check-prefix=LATENCY
